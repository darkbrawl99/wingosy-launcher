...
impl Database {
    ...
    pub fn get_emulator_for_game(&self, game_id: i64, platform_id: &str) -> Result<Option<EmulatorConfig>> {
        ...
        if let Ok(config) = stmt.query_row(params![game_id], |row: &rusqlite::Row| {
            Ok(EmulatorConfig {
        ...
        if let Ok(config) = stmt.query_row(params![platform_id], |row: &rusqlite::Row| {
            Ok(EmulatorConfig {
                platform_id: row.get("platform_id")?,
                game_id: row.get("game_id")?,
                emulator_id: row.get("emulator_id")?,
        ...
        if let Ok(config) = stmt.query_row([], |row: &rusqlite::Row| {
            Ok(EmulatorConfig {
                platform_id: row.get("platform_id")?,
                game_id: row.get("game_id")?,
                emulator_id: row.get("emulator_id")?,
    ...
    pub fn get_platform_default_emulator(&self, platform_id: &str) -> Result<Option<EmulatorConfig>> {
        ...
        let config = stmt
            .query_row(params![platform_id], |row: &rusqlite::Row| {
                Ok(EmulatorConfig {
                    platform_id: row.get("platform_id")?,
                    game_id: row.get("game_id")?,
                    emulator_id: row.get("emulator_id")?,
